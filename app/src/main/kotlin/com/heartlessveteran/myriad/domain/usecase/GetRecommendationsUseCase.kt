...
class GetRecommendationsUseCase @Inject constructor(
    ...
    operator fun invoke(
        ...
        limit: Int = 10
    ): Flow<Result<List<ContentRecommendation>>> = flow {
        emit(Result.Loading)
     ...
     */
    fun getTrendingRecommendations(
        limit: Int = 10
    ): Flow<Result<List<ContentRecommendation>>> = flow {
        emit(Result.Loading)
     ...
     */
    private fun analyzeUserPreferences(userManga: List<com.heartlessveteran.myriad.domain.entities.Manga>): UserPreferences {
        ...
        val preferredGenres = genreCounts.entries.sortedByDescending { it.value }.take(5).map { it.key }
        val preferredAuthors = authorCounts.entries.sortedByDescending { it.value }.take(3).map { it.key }
     ...
     */
    private fun getRecommendationsForGenre(genre: String, limit: Int): List<ContentRecommendation> {
        return when (genre.lowercase()) {
            "action" -> listOf(
                ContentRecommendation("action_1", "One Piece", 0.92f, "Epic action adventure", listOf("Action", "Adventure")),
                ContentRecommendation("action_2", "Naruto", 0.88f, "Classic ninja action", listOf("Action", "Martial Arts")),
                ContentRecommendation("action_3", "Dragon Ball", 0.85f, "Legendary battle manga", listOf("Action", "Fighting"))
            )
            "romance" -> listOf(
                ContentRecommendation("romance_1", "Kaguya-sama", 0.90f, "Romantic comedy masterpiece", listOf("Romance", "Comedy")),
                ContentRecommendation("romance_2", "Horimiya", 0.87f, "Heartwarming school romance", listOf("Romance", "School")),
                ContentRecommendation("romance_3", "Your Name", 0.84f, "Beautiful love story", listOf("Romance", "Drama"))
            )
            "fantasy" -> listOf(
                ContentRecommendation("fantasy_1", "Made in Abyss", 0.91f, "Dark fantasy adventure", listOf("Fantasy", "Adventure")),
                ContentRecommendation("fantasy_2", "Re:Zero", 0.88f, "Isekai fantasy drama", listOf("Fantasy", "Drama")),
                ContentRecommendation("fantasy_3", "Overlord", 0.85f, "Dark fantasy strategy", listOf("Fantasy", "Action"))
            )
            "slice of life" -> listOf(
                ContentRecommendation("slice_1", "K-On!", 0.89f, "Music and friendship", listOf("Slice of Life", "Music")),
                ContentRecommendation("slice_2", "Yuru Camp", 0.86f, "Relaxing camping adventures", listOf("Slice of Life", "Outdoor")),
                ContentRecommendation("slice_3", "Lucky Star", 0.83f, "Daily life comedy", listOf("Slice of Life", "Comedy"))
            )
            else -> listOf(
                ContentRecommendation("general_1", "Attack on Titan", 0.90f, "Critically acclaimed series", listOf("Action", "Drama")),
                ContentRecommendation("general_2", "Death Note", 0.88f, "Psychological thriller", listOf("Thriller", "Supernatural"))
            )
        }.take(limit)
    }
     ...
     */
    private fun getRecommendationsForAuthor(author: String, limit: Int): List<ContentRecommendation> {
