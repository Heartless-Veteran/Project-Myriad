...
class TrackingServiceImpl(
    ...
    override suspend fun updateProgress(
        ...
        return try {
            ...
            Log.i(
                TAG,
                "Updated progress for $serviceId:$trackingId - Progress: $progress, Status: $status, Score: $score",
            )
            Result.Success(Unit)
        } catch (e: Exception) {
    ...
    override fun bulkSync(serviceId: String): Flow<SyncResult> =
        flow {
            try {
    ...
    private fun isServiceAuthenticated(serviceId: String): Boolean =
        preferences.getBoolean("${serviceId}_authenticated", false)
        ...
        return statuses.flatMap { currentStatus ->
            (1..3).map { index ->
                TrackingEntry(
                    ...
                    serviceId = serviceId,
                    title = "${type.name.lowercase().replaceFirstChar {
                        it.uppercase()
                    }} ${currentStatus.name.lowercase()} $index",
                    ...
                    totalEpisodes = if (type == TrackingContentType.ANIME) 24 else null,
                    score =
                        if (currentStatus == TrackingStatus.COMPLETED) {
                            kotlin.random.Random.nextFloat() * 3f + 7f
                        } else {
                            null
                        },
                )
