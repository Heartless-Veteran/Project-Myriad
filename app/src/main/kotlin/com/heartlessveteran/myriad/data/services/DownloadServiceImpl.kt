...
class DownloadServiceImpl(
    ...
    private val downloadBaseDir: File
        get() = File(context.getExternalFilesDir(null), DOWNLOAD_DIR_NAME).apply {
            if (!exists()) mkdirs()
        }
    ...
    override fun getMangaDownloads(mangaId: String): Flow<List<DownloadTask>> {
        return _downloadQueue.map { tasks ->
    ...
    override suspend fun setDownloadPreferences(maxConcurrent: Int, wifiOnly: Boolean): Result<Unit> {
        return try {
    ...
    private fun processDownloadQueue() {
        serviceScope.launch {
            val queuedTasks = _downloadQueue.value.filter { task ->
    ...
    private fun updateTaskStatus(
        ...
        if (taskIndex >= 0) {
            ...
            currentQueue[taskIndex] = task.copy(
                status = status,
