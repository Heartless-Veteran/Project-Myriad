...
class SourceServiceImpl : SourceService {
    ...
    override fun getAvailableSources(): List<ContentSource> {
        return availableSources
    }
    
    override fun getEnabledSources(): List<ContentSource> {
        return availableSources.filter { enabledSourceIds.contains(it.id) }
    }
    ...
    override fun searchMangaAcrossSources(
        ...
        val searchSources = sources?.let { requestedSources ->
    ...
    override suspend fun getLatestManga(sourceId: String, page: Int): Result<List<Manga>> {
    ...
    override suspend fun getPopularManga(sourceId: String, page: Int): Result<List<Manga>> {
        return when (sourceId) {
    ...
    override suspend fun getMangaDetails(sourceId: String, mangaId: String): Result<Manga> {
        return when (sourceId) {
    ...
    override suspend fun getMangaChapters(sourceId: String, mangaId: String): Result<List<MangaChapter>> {
        return when (sourceId) {
    ...
    override suspend fun configureSource(sourceId: String, settings: Map<String, Any>): Result<Unit> {
     ...
     */
    private suspend fun searchMangaInSource(sourceId: String, query: String): Result<List<Manga>> {
        return when (sourceId) {
    ...
    private fun generateSampleManga(prefix: String, count: Int): List<Manga> {
    ...
    private fun generateSampleChapters(mangaId: String, count: Int): List<MangaChapter> {
        return (1..count).map { chapterNumber ->
            MangaChapter(
                id = UUID.randomUUID().toString(),
                mangaId = mangaId,
